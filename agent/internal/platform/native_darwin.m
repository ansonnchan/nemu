...
-(void)action:(NSMenuItem *)sender { command=(int)sender.tag; }
@end
...
void nemu_init(void) {
 ...
 [nc addObserverForName:NSWorkspaceWillSleepNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *n){sleeping=YES;command=10;}];
 [nc addObserverForName:NSWorkspaceDidWakeNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *n){sleeping=NO;command=11;}];
 [nc addObserverForName:NSWorkspaceSessionDidResignActiveNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *n){sleeping=YES;command=10;}];
 [nc addObserverForName:NSWorkspaceSessionDidBecomeActiveNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *n){sleeping=NO;command=11;}];
 [NSApp finishLaunching];
}
int nemu_pump(void) { @autoreleasepool { NSEvent *ev;while((ev=[NSApp nextEventMatchingMask:NSEventMaskAny untilDate:[NSDate distantPast] inMode:NSDefaultRunLoopMode dequeue:YES])){[NSApp sendEvent:ev];}[[NSRunLoop currentRunLoop] runUntilDate:[NSDate dateWithTimeIntervalSinceNow:0.01]];int c=command;command=0;return c;} }
...
char *nemu_secret_read(const char *key){@autoreleasepool{NSMutableDictionary *q=query(key);q[(__bridge id)kSecReturnData]=@YES;CFTypeRef result=NULL;OSStatus s=SecItemCopyMatching((__bridge CFDictionaryRef)q,&result);if(s!=errSecSuccess)return NULL;NSData *data=CFBridgingRelease(result);return strdup([[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding] UTF8String]);}}
